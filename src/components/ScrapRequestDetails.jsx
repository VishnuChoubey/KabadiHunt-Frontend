...
const acceptOrder = async () => {
    try {
        const res = await fetch(`/api/order/${orderId}/accept`, {
    ...
    try {
        const res = await fetch(`/api/order/${orderId}/reject?reason=${encodeURIComponent(selectedReason)}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${localStorage.getItem("access")}`,
        ...
        <div className="container">
            <div className="left-section">
                <div className="user">
                    ...
                    <img src={"http://localhost:8080" + userData.user.profileImageUrl || '../assets/default.jpg'} alt="User" />
                </div>

                <table>
                    <tbody>
                        <tr><th>Content</th><th>Details</th></tr>
                        <tr><td>Customer name:</td><td>{userData.user.username.toUpperCase()}</td></tr>
                        <tr><td>Scrap Type:</td><td>{userData.itemType}</td></tr>
                        <tr><td>Date to reach:</td><td>{userData.date}</td></tr>
                        <tr><td>Scrap quantity:</td><td>{userData.weight / 1000} Kg</td></tr>
                        <tr><td>Address:</td><td>{userData.longitude}    {userData.latitude}</td></tr>
                    </tbody>
                </table>
                ...
                <img
                    className="scrap-img"
                    src={"http://localhost:8080" + userData.imagePath || '../assets/default.jpg'}
                    alt="Scrap"
                ...
                <div id="myModal" className="modal">
                    ...
                    <img className="modal-content" src={"http://localhost:8080" + userData.imagePath || '../assets/default.jpg'} alt="Scrap Zoomed" />
                </div>
